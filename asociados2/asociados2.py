...
class grupo_familiar(osv.osv):
    ...
    _rec_name = 'nombre'
    def _edad(self, cr, uid, ids, field_name, arg, context=None):
	    hoy=date.today()
	    edad={}
	    fromato="%x"
	    s = "1976-05-26T00:00:00"
	    for o in self.browse(cr, uid, ids, context=None):
		    edad[o.id]=datetime.date.strftime(s[:10], "%Y-%m-%d")
	    return edad
    def _edadss(self, cr, uid, ids, field_name, arg, context=None):
	    hoy=date.today()
	    edad={}
	    s = "1976-05-26T00:00:00"
	    fromato="%x"
	    for o in self.browse(cr, uid, ids, context=None):
		    edad[o.id]=hoy.strftime(fromato) - o.fecha_nac
	    return edad
    _columns = {
        'nombre': fields.char('Nombres', size=100, required=True),
        'edad': fields.function(_edad, string='Edad', type="char",size=100, store=True, help=' edad'),
...
class partido_politico(osv.osv):
    ...
    _columns = {
        'nombre': fields.char('Partido Politico', size=100, required=True),
    }
...
class movimiento_grupo(osv.osv):
    ...
    _columns = {
        'nombre': fields.char('Movimiento', size=100, required=True),
        'grupo_familiar_id': fields.many2one('grupo_familiar', 'Persona'),
    }
...
class enf_dis_grupo(osv.osv):
    ...
    _columns = {
        ...
        'enfer_disc':fields.selection([('enf', 'Enfermedad'), ('dis', 'Discapacidad')], 'Produce', help="Se colocara el Estatus del Fundo Zamorano en la produccion (Productiva y No Productiva).", required=True, states={'productiva': [('readonly', False)]}),
    }
    _defaults = {
       'enfer_disc': 'enf',
